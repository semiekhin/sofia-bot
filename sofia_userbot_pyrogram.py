...
LOG_PATH = os.path.join(SOFIA_PATH, "userbot.log")
...
async def broadcast_to_observers(
    app, user_name: str, user_id: int, user_msg: str, sofia_msg: str
):
...
{sofia_msg}"""
        ...
        await app.send_message(OBSERVER_CHAT_ID, text, parse_mode=enums.ParseMode.HTML)
...
async def process_with_sofia(user_id: int, user_name: str, message: str) -> str:
    ...
    try:
        ...
        response = await generate_with_openai(
            user_name, message, history_list, action_context, examples_text
        )
...
async def generate_with_openai(
    user_name: str, message: str, history: list, action_context: dict, examples: str
) -> str:
...
async def start_smart_dialog(target: str, lead_info: dict = None) -> bool:
    ...
    try:
        ...
        log(
            f"📊 Initial state: goal={client_state.goal}, location={client_state.location}, budget={client_state.budget}"
        )
        ...
        first_message = await generate_first_message(
            user_name, action_context, lead_info
        )
        ...
        if action.value.startswith("ask_"):
            ...
            state_manager.update_state(
                user_id, {"slot_attempts": client_state.slot_attempts}
            )
...
async def generate_first_message(
    user_name: str, action_context: dict, lead_info: dict = None
) -> str:
    ...
    if lead_info:
        if lead_info.get("source") == "avito":
            ...
            source_context = (
                f"Клиент откликнулся на объявление: {lead_info['ad_text'][:100]}"
            )
...
"""
    ...
    messages = [{"role": "user", "content": "Сгенерируй первое сообщение клиенту."}]
...
async def main_daemon():
    ...
    try:
        ...
        if "AUTH_KEY_UNREGISTERED" in error_msg or "401" in error_msg:
            ...
            log(f"   2. rm {SOFIA_PATH}/sofia_pyrogram.session")
            log(f"   3. python {SOFIA_PATH}/sofia_userbot_pyrogram.py")
