...
- true — ТОЛЬКО при явной просьбе ОТПРАВИТЬ/СКИНУТЬ материалы КАК АЛЬТЕРНАТИВА созвону:
  ✓ "скиньте варианты" (после предложения созвона)
  ✓ "отправьте подборку, созвон не нужен"
  ✓ "лучше в переписке, пришлите что есть"
  ✓ "не хочу созвон, просто покажите варианты"
- false — во всех остальных случаях, особенно:
  ✗ "давай варианты и там и там" — это ОТВЕТ на вопрос "море или горы?", location: "all"
  ✗ "покажите оба" — это ответ на выбор, НЕ просьба материалов
  ✗ "и то и то посмотрим" — ответ "оба варианта"
  ✗ "давайте посмотрим разные" — ответ на квалификационный вопрос

ВАЖНО: Смотри КОНТЕКСТ — если бот задал вопрос с выбором (море/горы, аренда/рост, и т.д.) 
и клиент ответил "давай оба/и то и то/варианты" — это ОТВЕТ на вопрос, НЕ wants_materials!
