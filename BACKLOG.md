...
- [x] ~~**🔴 fail2ban/UFW на VPS 185.207.66.201**~~ — **UFW выполнено 17.04**, **fail2ban выполнено 18.04** (см. «Завершено 18.04.2026»). Двухслойная защита активна.
...
- [x] **FAIL2BAN_DEPLOY_v1** — установка fail2ban на VPS sofia-voice, два jail (sshd+asterisk), banaction=ufw, 9 SSH-сканеров забанены с первого старта. UFW + fail2ban = двухслойная защита. `ignoreip` включает 46.229.221.93 (Telfin), 72.56.64.91 (главный сервер), 77.106.252.172 (личный Сергея). `[asterisk]` — явный `backend=auto` на `messages.log` (journal у asterisk пустой). Детали и уроки — SESSION_LOG 18.04 + CLAUDE.md «Уроки из ошибок».
