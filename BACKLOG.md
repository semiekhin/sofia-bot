...
- [ ] **🔴 Пилотная живая серия RIZALTA inbound — в процессе** — 18.04 вечер: первые 3 звонка от тестера +79160322905 пришли (UUIDs `e561a7c6`, `94ea75b1`, `b2786722`). Все записаны MixMonitor'ом. Остаётся ~25-27 звонков от 4-5 ещё тестеров. После завершения серии (ожидается 19.04): Сергей прослушивает WAV + cross-ref с journalctl, определяет rate of drop-offs / Sofia-initiated refusals / audio dropouts. Решение о Option A prompt hardening принимается based on tester-call attrition pattern.
...
- [ ] **Audio dropouts diagnostic** — Сергей слышит что Sofia иногда «глотает слова». В journalctl evidence чистый (0 RTP warnings, 0 underrun, 0 traceback). Гипотезы: `asyncio.sleep(0.018)` TTS pacing под CPU нагрузкой на 1 vCPU / prompt-artefacts / TTS generation boundary. После пилота (WAV available): пройтись по записям, найти exact timestamps дропов, cross-ref с journalctl. Если дропы в WAV тоже — не downstream SIP/PSTN, наша сторона. Эффорт диагностика: 2-3 часа post-пилот.
...
- [ ] **🔴 Апгрейд VPS sofia-voice до 2-4 vCPU** — текущее 1 vCPU = hard bottleneck для concurrent calls. Realistic safe: 3-4 одновременно (Silero VAD singleton под GIL + `intra_op_num_threads=1`). Если пилот успешен и начинается реальный cold-base обзвон с >5 параллельных линий — требуется апгрейд. Yandex quotas (STT 10 concurrent streams, LLM 10 RPS) не узкое место. Эффорт: 1 час (upgrade plan + reboot), стоимость — дополнительный hosting fee. Ждёт бизнес-решения Сергея после пилота.
