...
- [x] ~~**Yandex STT v3 gRPC streaming (Phase 2A+2B+2C)**~~ — выполнено 17.04. Phase 2A: grpcio 1.80.0 + yandex_proto + `yandex_stt_grpc.py` на VPS, smoke-tested. Phase 2B: bifurcate через method-pointer (`_process_audio`), `text_override` в `_process_turn`, STT_MODE=grpc задеплоен, live-test `fe7eed5c` — длинные фразы с микропаузами склеены целиком, barge-in + continuation сохранён. Phase 2C: continuous streaming (убрана блокировка при `_is_responding`), TTS-echo guard в `_on_grpc_*` через `_tts_playing and not _cancel_playback`, live-test `4b109925` — 8 turns / 3 barge-in / 0 false turns / phone AEC работает. **Осталось Phase 2D** — 2 новые P1 ниже. Оценка убираемой логики скорректирована: ~180-210 строк (не 400+). Подробности: `docs/VOICE_RESEARCH_2026_04_16.md` идея #1, SESSION_LOG 17.04 Phase 2B+2C.
- [ ] **Yandex EouSensitivity=HIGH тест (Phase 2D)** — главный bottleneck пауз в gRPC-звонках: `DefaultEouClassifier` server-side ждёт ~2240мс после стабилизации текста перед EOU. Одна строка правки в `yandex_stt_grpc.py::_request_iterator` — добавить `DefaultEouClassifier(type=HIGH)` в `StreamingOptions.eou_classifier`. Deploy + один контрольный звонок. Ожидаемый gain: **−500-1000мс user pause**. Риск: HIGH может false-positive'нуть на длинных фразах с микропаузами — если регрессия на фразе типа «а так хорошо а какая говорите минимальная цена не услышал» (turn 5 из 4b109925) → откат env-переменной или обратно на DEFAULT. Эффорт: **30 минут**.
- [ ] **Post-barge-in ack timer** — решает pre-existing deadlock-класс проблем при коротких barge-in: клиент перебивает коротко (~200мс), ожидает ack от Sofia, Sofia молчит (нет EOU триггера от Yandex/REST) → 7+ секунд тишины пока клиент не скажет «алло». Подтверждено в REST `71b461f4` (8.36с) и gRPC `4b109925` BI#2 (7.3с). Решение: в `_process_turn` finally при exit через barge-in — запустить `asyncio.create_task` с `asyncio.sleep(3)` → если за 3с не появился новый turn (`_is_responding=False и _last_user_text не обновлялся`) → Sofia говорит короткую ack-фразу «я вас слушаю?» / «продолжайте пожалуйста». Пер-режимно независимо (STT_MODE=rest/grpc). Эффорт: **1-2 часа** + live-тест. Рассматривается в связке с промптом (где брать текст ack-фразы — хардкод или LLM?).
