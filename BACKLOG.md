...
- [ ] **🔴 SSH alias sofia-voice починить** — алиас перестал корректно работать (уточнить симптомы от Сергея). Разведка: `~/.ssh/config` на локалке — entry `Host sofia-voice`, проверить HostName/User/IdentityFile/ServerAliveInterval. На VPS `~/.ssh/authorized_keys` актуальность. Может быть связано с истёкшим ключом или сменой IP/порта. Эффорт: 15-30 минут разведка + фикс. **Блокер** для всех операций на VPS sofia-voice (dial.py запуск, логи, любой deploy) — починить первым.
- [ ] **🔴 Запись звонков (MixMonitor WAV)** — приоритет перед живой серией: без записи не прослушать проблемные звонки post-mortem. Добавить `MixMonitor(${REC_DIR}/${CALL_UUID}.wav,)` в оба dialplan context'а (`[from-telphin]` + `[outbound-audiosocket]`) между `Set(CALL_UUID)` и `AudioSocket`. Папка `/var/lib/asterisk/recordings/YYYY-MM-DD/` с `asterisk:asterisk 0755`. Ночной cron (03:00) конвертирует WAV>12ч в Opus через ffmpeg + удаляет WAV, 03:30 удаляет Opus>30д. logrotate не нужен (cron управляет). Spec ранее отложен в пользу outbound priority — сейчас ключевой. Эффорт: 1-1.5 часа + live-верификация одним звонком.
- [ ] **🔴 Начать живую тестовую серию RIZALTA** — после fix SSH alias и deploy MixMonitor: инициировать серию реальных outbound звонков на клиентскую базу для сбора обратной связи. Цель: собрать метрики реального UX (deadlock-частота, barge-in поведение, save_message latency распределение) перед дальнейшими оптимизациями. Предварительно: (a) legal OK, (b) готовый список номеров, (c) WAV recording активен (см. выше), (d) post-call review workflow — Сергей слушает WAV + transcript, отмечает проблемные фрагменты. Серия не full-campaign — пилотная 10-20 звонков для оценки.
