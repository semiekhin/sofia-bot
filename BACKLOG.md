...
- [ ] **Outbound dialing — Sofia сама инициирует звонок** — сейчас Sofia принимает входящие через Asterisk AudioSocket. Для RIZALTA-кампаний нужен исходящий обзвон: чтение списка номеров из файла/CRM → `originate` через Asterisk AMI → AudioSocket bridge → обычный pipeline. Технически требует: AMI-клиент в Python (asterisk-ami или asterisk-amy), логика выбора следующего номера (rate limit, не-дозвоны, callback schedule), интеграция с Bitrix для финализации лида. Эффорт: 2-3 дня. **Бизнес-зависимость:** есть готовый список для прозвона? legal constraints на cold calling?
