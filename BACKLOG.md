...
- [~] **🔴 fail2ban/UFW на VPS 185.207.66.201** — **UFW часть выполнена 17.04**: whitelist 22/tcp + 46.229.221.93:5090/udp (Telfin) + 10000-20000/udp (RTP), default deny incoming. Рост full.log 44 KB/s → **0 B/s** (100% сканерного трафика блокируется на SYN). Telfin trunk Active, звонки проходят. **fail2ban часть отложена в P1** (не срочно, UFW даёт полное решение).
...
- [ ] **fail2ban deploy на VPS** — UFW уже даёт 100% защиту (см. закрытый P0). fail2ban = multi-layered defense. Контракт готов в истории чата 17.04. Два jail: sshd (`maxretry=10`, `bantime=3600`), asterisk (`port=5090,5060`, `maxretry=5`). `banaction=ufw`, `ignoreip=127.0.0.1/8 ::1 46.229.221.93`. Проверить наличие `/etc/fail2ban/filter.d/asterisk.conf` (в Ubuntu 22/24 может отсутствовать). Эффорт: 0.5 дня.
- [ ] **Yandex STT v3 gRPC streaming** — заменить REST v1 на bidirectional gRPC stream (`Recognizer/RecognizeStreaming`), partial+final результаты, EOU от сервера. Proto в `yandex_proto/` уже есть. **Архитектурное упрощение**: server-side EOU уберёт нашу кастомную барж-ин логику целиком (VAD_SILENCE_THRESHOLD, 10-frame confirm, sliding-silence reset — всё это было заплатки). Gain: −150-300ms (убирает REST TLS handshake + VAD 500ms timeout). Эффорт: 1-2 дня. Подробности: `docs/VOICE_RESEARCH_2026_04_16.md` идея #1.
