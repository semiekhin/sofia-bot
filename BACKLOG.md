...
- [~] **Yandex STT v3 gRPC streaming** — **Phase 2A закрыт 17.04** (YANDEX_STT_V3_INFRA_v1): grpcio 1.80.0 + yandex_proto на VPS, `/opt/sofia-voice/yandex_stt_grpc.py` (callback-based `YandexSTTStream`), smoke test с 320B silence подтвердил auth + bidirectional stream + clean close. `STT_MODE=rest` в .env (default). **Осталось Phase 2B**: bifurcate в `voice_asterisk.py::_handle_packet` под `STT_MODE=grpc`, fallback на REST при `is_broken`, A/B качества `general:rc` @ 8kHz vs REST v1, живой звонок. Silero остаётся как early-warning barge-in, EOU с сервера Yandex. Эффорт Phase 2B: 1-2 дня. 8 open questions — см. Build Report YANDEX_STT_V3_RECON_v1 в истории сессии 17.04. Proto-оценка убираемой логики скорректирована: ~180-210 строк (не 400+). Подробности также: `docs/VOICE_RESEARCH_2026_04_16.md` идея #1.
