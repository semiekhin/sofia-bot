...
class ClientState:
    ...
    # NEW: Bitrix integration
    finance_interested: bool = False
    manager_active: int = 0  # 1 = менеджер перехватил, София молчит
    ...
    def summary(self) -> str:
        ...
        if self.budget:
            parts.append(
                f"Бюджет: {self.budget/1_000_000:.1f} млн ({self.budget_confidence})"
            )
        if self.payment_type:
            parts.append(
                f"Оплата: {self.payment_type} ({self.payment_type_confidence})"
            )
        if self.lpr:
...
class StateManager:
    ...
    def _init_db(self):
        ...
        conn.execute("""
            CREATE TABLE IF NOT EXISTS client_state (
                ...
                source_object TEXT,
                finance_interested BOOLEAN DEFAULT 0,
                manager_active INTEGER DEFAULT 0
            )
        """)
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_client_state_user ON client_state(user_id)"
        )
        conn.commit()
        conn.close()
    ...
    def get_state(self, user_id: int) -> ClientState:
        ...
        state = ClientState(
            ...
            call_refused=(
                bool(row["call_refused"]) if "call_refused" in row.keys() else False
            ),
            ...
            slot_attempts=(
                json.loads(row["slot_attempts"]) if row["slot_attempts"] else {}
            ),
            # NEW v2.0 fields
            ...
            call_proposal_count=(
                row["call_proposal_count"] if "call_proposal_count" in row.keys() else 0
            ),
            materials_request_count=(
                row["materials_request_count"]
                if "materials_request_count" in row.keys()
                else 0
            ),
            dialog_finished=(
                bool(row["dialog_finished"])
                if "dialog_finished" in row.keys()
                else False
            ),
            ...
            call_readiness=(
                row["call_readiness"] if "call_readiness" in row.keys() else 0.5
            ),
            ...
            source_object=(
                row["source_object"] if "source_object" in row.keys() else None
            ),
            finance_interested=(
                bool(row["finance_interested"])
                if "finance_interested" in row.keys()
                else False
            ),
            manager_active=(
                int(row["manager_active"]) if "manager_active" in row.keys() else 0
            ),
        )
        return state
    ...
    def _save_state(self, state: ClientState):
        ...
        """,
            (
                state.user_id,
                state.goal,
                state.goal_confidence,
                state.location,
                state.location_confidence,
                state.budget,
                state.budget_confidence,
                state.payment_type,
                state.payment_type_confidence,
                state.first_payment,
                state.first_payment_confidence,
                state.lpr,
                state.lpr_confidence,
                state.meeting_agreed,
                state.meeting_datetime,
                state.materials_sent,
                state.call_refused,
                state.current_question_type,
                state.current_objection,
                state.wants_materials,
                state.mentioned_location,
                state.mentioned_price,
                state.updated_at,
                ...
                state.usage,
                state.family,
                state.call_proposal_count,
                state.materials_request_count,
                state.dialog_finished,
                state.finish_type,
                state.friction,
                state.call_readiness,
                state.engagement,
                state.urgency,
                state.lead_type,
                state.source_object,
                state.finance_interested,
    ...
    def set_meeting_agreed(self, user_id: int, datetime_str: str = None):
        ...
        self.update_state(
            user_id, {"meeting_agreed": True, "meeting_datetime": datetime_str}
        )
