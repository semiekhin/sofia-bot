...
| `no_response_after_reminder` | 15 мин молчания → напоминание → ещё 15 мин | bot_server.py:842 |
...
| B: Напоминание | TIMEOUT_REMINDER | **15 мин** (env `REMINDER_TIMEOUT_MIN`, default 15) | Клиент отвечал, потом замолчал | Отправка напоминания |
| B+: После напоминания | TIMEOUT_AFTER_REMINDER | 15 мин | Молчание после напоминания | finalize с "no_response_after_reminder" |

> ⏱️ **20.05.2026:** TIMEOUT_REMINDER сокращён 60 → 15 мин через env `REMINDER_TIMEOUT_MIN` (коммит `102b2b33`). Полный цикл «клиент замолчал → напоминание → финализация» теперь **30 мин** (было 75). Действует и для Telegram-бота, и для Radist-канала (`radist_send_reminder`). 0 = старое поведение неприменимо; default 15.
