...
### 17.02.2026: @username → "ник в Telegram"
**Проблема:** В виджете "@username" рендерится как кликабельная синяя ссылка. Клиент жмёт вместо того чтобы написать свой ник.
**Решение:** sed замена в web_api.py строка 373: «Напишите ваш ник в Telegram» вместо «@username»
**Почему:** Веб-чат парсит @ как ссылку. В Telegram это не проблема, в HTML — да.

### 17.02.2026: Аудит — расхождения документации и кода
**Обнаружено:**
- Bitrix send_lead_to_bitrix() только в web_api.py, лиды из Telegram/Max теряются
- radist_gateway get_history(limit=8) vs 100 в остальных — контекст теряется
- sofia_prompt_v2.py: ставка ЦБ "~16%", в web_api.py: "15.5%" — расхождение
- radist_gateway: мёртвый импорт from sofia_prompt import get_system_prompt (v1)
**Решение:** SOFIA_PROJECT_MAP.md обновлён, задачи #8-11 добавлены
