...
### 19.01.2026: Финансовый модуль v1.0

**Задача:** Sofia должна уметь показывать расчёт доходности недвижимости vs депозит, когда клиент интересуется выгодой.

**Архитектура решения:**
1. `finance_data.json` — хранит данные (ставки ЦБ, рост недвижимости, аренду)
2. `finance_calculator.py` — функции расчёта + `add_finance_context()`
3. Extractor — извлекает триггеры `finance_interest`, `deposit_mention`
4. State — хранит флаг `finance_interested` (чтобы не "забывать" интерес между сообщениями)
5. bot_server/userbot — формирует `finance_context` с расчётом
6. sofia_prompt — директивы как использовать расчёт в ответе

**Логика срабатывания:**
- Если `finance_interest` или `deposit_mention` + бюджет известен → показать расчёт
- Если `finance_interest` или `deposit_mention` + бюджет НЕ известен → переключить на ASK_BUDGET
...
**Известная проблема:** Extractor не извлекает бюджет из комбинированных сообщений ("15. какая доходность?"). Нужно доработать.

---

### 19.01.2026: Уточнение wants_materials

**Проблема:** Бот слишком рано завершал диалог (FINISH_WITH_MATERIALS), потому что "можете показать варианты?" считалось как просьба отправить подборку.

**Решение:** Уточнили правило в Extractor:
- `wants_materials: true` — ТОЛЬКО при явной просьбе ОТПРАВИТЬ: "скиньте", "отправьте", "пришлите"
- `wants_materials: false` — "покажите варианты" (это запрос на созвон/показ, не на отправку)

**Почему:** "Покажите" ≠ "Отправьте". Клиент хочет увидеть, а не получить PDF в переписку.

---

### 19.01.2026: Правило "стройка = growth"

**Проблема:** Когда клиент говорил "аренда, можно стройку" — Sofia переспрашивала про стратегию.

**Решение:** Добавили в Extractor связанное извлечение:
- "стройка", "на стройке", "можно стройку" → strategy: growth (капитализация)
- "аренда + стройка" → strategy: any (не переспрашивать)

**Почему:** Стройка всегда предполагает рост стоимости — это очевидно для клиента, не нужно переспрашивать.

---

### 19.01.2026: Проблема с Extractor и комбинированными сообщениями

**Проблема:** Клиент пишет "15. какая примерно доходность?" — Extractor извлекает только `finance_interest: True`, но НЕ извлекает `budget: 15000000`.

**Причина:** Extractor воспринимает "15." как начало предложения (нумерация), а не как ответ на вопрос о бюджете.

**Статус:** НЕ РЕШЕНО. Требует доработки промпта Extractor — нужно правило: извлекать факты ДАЖЕ из комбинированных сообщений с встречным вопросом.

**Возможное решение:** Добавить в промпт Extractor:
```
ВАЖНО: Если клиент одновременно отвечает на вопрос И задаёт свой вопрос — извлекай ОБА:
- "15. какая доходность?" → budget: 15000000 + finance_interest: true
- "Сочи, а там какие цены?" → location: sochi + question_type: price
```
