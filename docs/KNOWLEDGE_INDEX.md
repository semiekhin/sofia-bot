...
- **Статус:** Исследование, прототипа пока нет
...
- **⚠️ Архитектурная заметка:** voice_mode — намеренное временное упрощение. Отключены: Extractor (state не обновляется), Analyzer, reasoning, RAG 10→3, ответ макс 2 предложения. Это компромисс скорость/качество. По мере оптимизации — возвращать компоненты по одному (первый кандидат: Extractor async в фоне).
- **Файлы:**
