...
### Конфликт «ПЕРВАЯ РЕПЛИКА vs Случай 4» в `prompt_addon` (после 19.05)

Новый prefilled-текст «Здравствуйте, **отправьте презентацию** АК «Атлантис»» — это **буквально Случай 4 в блоке ПРЕЗЕНТАЦИЯ (PDF)** («Клиент прямо попросил презентацию» → отправь), но одновременно **первое сообщение**, на которое блок ПЕРВАЯ РЕПЛИКА говорит «отвечай мягко и переходи к квалификации». Два правила в прямом конфликте, разрешается LLM (`gpt-5.2`) на лету. Из кода нельзя предсказать. **Live-smoke 19.05+:** если Sofia отправляет PDF → конфликт de-facto разрешён в пользу Случая 4, не правим. Если квалификация без PDF → нужна точечная правка `prompt_addon` (одно предложение-исключение в блоке ПЕРВАЯ РЕПЛИКА: «если клиент в первом сообщении прямо просит презентацию — см. блок ПРЕЗЕНТАЦИЯ Случай 4»).
