...
- `config/source_objects.py`: маппинг объектов. `atlantis`: keywords `[атлантис, atlantis, #ATL]`, context из `objects/atlantis_context.md`, `prompt_addon` = не запрашивать контакты. `rizalta` (с 29.04, коммит `9947721d`): keywords `[ризалта, rizalta, rizalta resort]` (узкие — без `белокуриха` чтобы не false-match'нуть на упоминание города), context из `objects/rizalta_context.md`, `presentation_url` = `presentation_RIZALTA.pdf`. **С 01.06 у rizalta есть `prompt_addon`** (DEV `45fcfda3` / PROD `765ccfa7`) — поведенческий блок параллельно atlantis (КОНТЕКСТ ВХОДА → ПЕРВАЯ РЕПЛИКА → ВЕДЕНИЕ → ПРЕЗЕНТАЦИЯ → ЗАПРОС КОНТАКТА/КАНАЛ → ПЕРЕДАЁТ МЕНЕДЖЕРУ); инъекция через `core/pipeline.py:282-283`, активен во всех текстовых каналах с `source_object=="rizalta"`. Карточка `rizalta_context.md` 01.06 почищена под структуру Atlantis (markdown-таблица, правило «цену сама не называет — вилка только по прямому запросу»). RIZALTA — **исключительно инвестиционный продукт**, личное проживание собственника не предусмотрено (см. карточку, обновлено `4d79a8ae`).
- **Команда `/restart` в Radist** (01.06, DEV `3c07cba7` / PROD `fe8c3070`): whitelisted-тестеры (`RESTART_WHITELIST` в `sofia_radist_gateway.py` — Сергей `-40076372`, Изосина `-42638488`, Юля Смм `-40927660`) пишут `/restart` в @SofiaOazis → перехват в `process_message_wrapper` до pipeline → полный сброс сессии (`reset_state` client_state + `DELETE radist_messages` + `DELETE radist_leads`) + подтверждение. Не-whitelisted → сообщение идёт в обычный pipeline. Только Radist-канал (в `bot_server.py` команды нет — P3).
...
scp -P 2222 root@72.56.64.91:/root/.claude/projects/-opt-sofia-gpt-dev/memory/MEMORY.md ~/projects_claude/Sofia/memory/
